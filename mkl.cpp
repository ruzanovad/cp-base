...
int bin_search::binary_search(vector<int> &a, int x) {
    int l = 0, r = a.size(); // у нас ответ в l, полуинтервалы
    while (r - l > 1) { // l < r-1, при ситуации [l, l+1) возможный ответ уже хранится в l
        int mid = (l + r) / 2; // [l, r)
        if (a[mid] <= x) {
            l = mid; // продолжаем искать в [mid, r)
        } else r = mid; // ищем в [l, mid)
    }
    if (a[l] == x)
        return l;
    else
...
int bin_search::first_element_greater_or_equal_to(vector<int> &a, int x) {
    int l = -1, r = a.size() - 1; // у нас ответ в r, полуинтервал (l, r]
    while (r - l > 1) { // логично искать с конца, потому что ищем бОльший элемент
        int mid = (l + r) / 2;
...
double bin_search::invariant_with_precision(double a, double b, double x, double eps) {
    double l = a, r = b;
    while (r - l > eps) {
        double mid = l + (r - l) / 2;
...
double bin_search::ternary_search_with_precision(double a, double b, double eps) {
    double l = a, r = b;
    while (r - l > eps) {
        double m1 = l + (r - l) / 3;
        double m2 = l + 2 * (r - l) / 3;
...
int bin_search::fast_search_in_array(vector<int> &a, int l, int r) {
    ...
    return a[rightIndex] == r ? rightIndex + 1 : rightIndex -
                                                 (a[leftIndex] == l ? a.size() - leftIndex - 1 : a.size() - leftIndex) +
                                                 1;

}


unsigned long long number_theory::gcd(unsigned long long int a, unsigned long long int b) {
    if (b == 0)
        return a;
    return gcd(b, a % b);
}

unsigned long long number_theory::gcd_no_recursion(unsigned long long int a, unsigned long long int b) {
    while (b != 0) {
...
number_theory::gcd_result number_theory::extended_gcd(unsigned long long int a, unsigned long long int b) {
    // база
    if (b == 0) {
...
vector<pair<unsigned long long, unsigned long long>> number_theory::factorize(unsigned long long int x) {
...
unsigned long long
number_theory::bin_pow(unsigned long long int a, unsigned long long int p, unsigned long long int mod) {
    if (p == 0ULL)
        return 0ULL;
    if (p == 1ULL)
        return a % mod;
...
void number_theory::linear_sieve(size_t n) {
    ...
    for (size_t y = 2; y <= n; ++y) {
        ...
        for (size_t j = 0; j < primes.size() && primes[j] < lp[y] && y * primes[j] <= n; ++j) {
